...
- `0024` — `failed_write` (table, RLS, policies, grants) and `record_failed_write`
  (**4.5b**). ⚠️⚠️ **ITS FIRST DECISION IS OPEN AND IT AMENDS §2.6 IF TAKEN AS
  recommended**: §2.6 says the row stores `adjustment_movement_id`, SINGULAR, and a
  downgrade of a multi-line sale writes one movement per line and per lot. The
  recommendation is an `adjustment_group_id`, as `record_transfer` already carries a
  `transfer_group_id` for the identical reason. See `docs/PLAN.md`, *Found while
  sizing step 4.5*. ⚠️ **The table must land with RLS on and every policy reaching a
  tenancy helper** — `supabase/pgtap/01_rls_coverage.sql` computes its plan, so it
  covers a new table the day it lands and goes red on arrival otherwise. And
  `revoke all on <table> from anon, authenticated` before the grants, or the table is
  born TRUNCATE-able by any signed-in cashier (3.1)
