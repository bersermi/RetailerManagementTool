...
- `0026` — `replay_failed_write` (**4.5c-ii**). The LAST migration in the database
  build: compensate the downgrade movements `0024`'s link names, dispatch on kind,
  re-run the original call under its original id through the path `0025` built, and
  stamp the `failed_write` row replayed
