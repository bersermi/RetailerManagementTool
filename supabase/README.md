...
because nothing is downstream of step 4.5). ⚠️⚠️ **The upper bound has moved TWICE, both on 2026-09-04**: 4d and then 4e
...
- ~~`0023` — failure path, all of it in one migration~~ — ⚠️⚠️ **STEP 4.5 WAS SIZED
  ...
  the first one that did not.** Nothing is downstream of step 4.5: steps 5–7 are the
...
- ~~`0026` — `replay_failed_write` (**4.5c-ii**)~~ — **APPLIED 2026-09-05**, see the
  table above. The LAST migration in the database build; **nothing is downstream of
  it**, because steps 5–7 are the client and ship none
