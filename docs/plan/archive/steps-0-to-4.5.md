# Archived plan — Steps 0 through 4.5 (closed)

⚠️ **THIS FILE IS PART OF THE PLAN, NOT A BACKUP OF IT.** It was cut out of
`docs/PLAN.md` on 2026-09-19, when that file reached 14,998 lines / ~313k tokens —
**larger than a context window**, so no session could read it and every session
grepped it instead. Steps 0–4.5 were closed and still being carried in full.

**Nothing here was edited, summarised or deleted.** These are the original lines,
in their original order, and the word "archive" means *closed*, not *superseded* —
unlike `archive/power-platform/`, which describes a system that is not being built.
Everything in this file describes the system that IS being built, and was true when
it was written.

## How the checks still find these rows

⚠️ **`docs/checks/plan-corpus.sh` assembles `docs/PLAN.md` + this directory into one
file, and every plan check reads THAT.** So a task row keeps resolving after its
step is archived. This works because every plan lookup in the repository is
**content-addressed** — `grep -F "| **task** |"` — and never by line number. Had one
check used a line offset, this split would not have been possible.

⚠️ **The working plan wins on a collision.** The corpus is working-plan-first and
`row()` takes the first match, so if a task id appeared in both, the LIVE row is
the one every check reads and the archived copy is inert.

## If you need something back

**Reading it is not "getting it back" — just read it, or let the corpus do it.**
Pull a section into `docs/PLAN.md` only when work has genuinely REOPENED, and then:

1. **Move it, do not copy it.** Two homes for one claim is how six of this
   repository's seven stale-copy defects happened. Cut from here, paste there.
2. **Say in the row why it reopened**, and on what date.
3. **Re-run `bash docs/checks/split-coverage.sh --all`** and the plan guards. A row
   that now exists twice will be caught by "row appears 2 times" — that assertion
   is the safety net for exactly this move, and it reads the corpus, so it sees
   both copies.
4. **If a step reopens with a schema change, it is a fix-forward migration** with
   the next free number. Migrations are append-only once applied; nothing in here
   can be un-applied by moving text between files.
