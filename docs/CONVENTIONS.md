...
eleventh and the module that owns the connectivity import; `5d-i` added a twelfth,
and it is the first whose subject is a TABLE rather than an RPC;
...
| `src/api/catalog.ts` | what the shop SELLS — four column lists, the EMBED across a composite foreign key, the price window as PostgREST filters, and the arithmetic that turns `price_per_base` into `$35 / kg`. ⚠️⚠️ Its two numerics are asked for as **`::text`**: a bare `numeric` arrives as a JSON number, which is a double, and `@tienda/money`'s `parseDecimal` refuses a number outright. ⚠️ It is the first module here whose subject is not an RPC and not the outbox — a TABLE read — and the first that has to decide *which* row is current | **yes**, and `app/test/api-catalog.test.ts` does |
