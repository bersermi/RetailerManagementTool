...
| **5d** | **The Productos screen — seeing what you sell** | ⚠️ **This is where the next piece of work is**, and nothing is waiting on you for it. ⚠️⚠️ **It is the first job in this whole project whose subject is a SCREEN and nothing else** — everything built since 15 September has been plumbing you could not look at, and this is the other kind. **What it is:** the grid of your product families, each as a tile; tap one and a sheet opens showing its variants and their prices. **Reading only — nothing on it changes anything yet**, which is deliberate and is the same shape as the last five jobs: something you can look at and correct before anything is built on top of it. ⚠️⚠️ **AND THIS IS THE ONE WHERE YOUR EYE IS THE ONLY INSTRUMENT.** Our tests can prove the app agrees with the database; they were deliberately never given the job of judging how something LOOKS, so on this one there is nothing that can tell us it is wrong except you opening it. ⚠️ **It may well be split before any of it is written**, the way six jobs have been since 18 September — the rule is to size it first, and a job with no test behind it is an argument for a smaller first piece rather than a bigger one |
| 5d–5h | The rest of the screens | Now — `5d` is next |
