...
| **5b-iii-d** | **The bell on the home screen, and letting them in** | ⚠️ **This is where the next piece of work is** — and nothing about it is waiting on you. **The row that closes the loop.** A notifications icon on Inicio with a count on it, the list of people asking, each one showing the **email as the heading and the name underneath** — your ruling of 2026-09-19 — and the button that admits them. ⚠️⚠️ **Admitting an Empleado makes you choose which store they work in, and it refuses to be left empty.** That is not fussiness: staff can only record things where they have been placed, so an employee admitted with nowhere attached opens the app and **every single thing they try to save is refused with no message at all** — which to the person holding the phone looks exactly like the app being broken. ⚠️ **It adds no database change** — everything it calls is already built, including the name lookup that went in today. ⚠️ **It will be re-sized on the day it is taken** — it carries a badge, a list and a picker, and four earlier rows here record a deferred half arriving bigger than it left |
