...
| **5b.8-i** | **Storing everybody's name, wherever they joined from** | ⚠️ **This is where the next piece of work is**, and nothing is waiting on you for it. It is the database change: every way a person can end up in your shop starts recording the name they signed up with. ⚠️ **Nothing shows it yet** — that is the next piece — and it is deliberate: storing it and showing it are two jobs, and doing them together is how the file said this was one task when it was three. ✅ **Your ruling of 2026-09-18 is built into it** — *"keep what they typed"* — so if somebody fixes their own name and you later invite them again, **the app keeps their version** instead of overwriting it with whatever Google sent. ⚠️ **That one is worth knowing because it cannot be undone later**: change it after this ships and any name already overwritten is simply gone |
| **5b.8-ii** | **Showing the name in the list of people** | **After the piece above.** The Ajustes list stops saying *Dueño* and *Encargado* where it can say a person's name instead. ⚠️ **This is the one that actually retires your no-names ruling of 2026-09-14** — you overturned it on 2026-09-18, and the name is not really on screen until this ships, so two internal checks keep holding the old rule until exactly this point |
| **5b.8-iii** | **Letting somebody fix their own name** | **After the first piece; it does not have to wait for the second.** A field in Ajustes where a person edits their own name. ⚠️ **It exists because of a real gap**: as the database stands today only a shop's OWNER may change a membership row, so an **Encargado** or **Empleado** who signed in with Google and came through as one word could never correct it. ⚠️ **The naive fix would also let them promote themselves to owner**, so it is done as one narrow operation that touches the name and nothing else |
