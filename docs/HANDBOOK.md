...
| **5b-ii-b** | **Inviting someone, and them joining** | **Split in two on 2026-09-18**, before any of it was written — it was bigger than the file said, the same way `5b-ii` was six days earlier. ⚠️ **Nothing was cut**: the two halves below are the same work, in the order a shop would actually do it |
| **5b-ii-b-1** | **Inviting someone: the button, and the code you read out** | ⚠️ **This is where the next piece of work is**, and nothing is waiting on you for it. You type the person's address, choose whether they are staff or a manager, and the app gives you a code to send them over WhatsApp. ⚠️ **The code is shown once and we never store it** — only a scrambled copy we can check it against — so if you lose it you invite them again, which takes one tap. ✅ **You will be told plainly when inviting the same person twice kills the first code**, because otherwise the code they are holding quietly stops working and looks like the app being broken. ⚠️ **After this half, nobody can yet USE the code** — that is the next row, and splitting it this way is what keeps each piece to one sitting |
| **5b-ii-b-2** | **Them joining: typing the code on their own phone** | **After `5b-ii-b-1`.** The second person installs the app, signs in, types the code, and is in your shop. ⚠️ **One decision here is worth a look** and it is written up in the plan: the code you send them is sixteen characters and the shop's own join code is eight, and they look identical otherwise — so the app will use ONE box and tell them apart by length, rather than asking a person which kind of code they were sent, which they have no way to know |
...
| **5b.8** | **Showing the name in the list of people** | **After inviting works.** This is the database change, and it is what makes your own row say your name instead of *Dueño*. It is also what retires your no-names ruling of 2026-09-14 — you overturned it on 2026-09-18, and this is the task that carries it through. ⚠️ **It grew the same day, from one look at the database**: as things stand only a shop's OWNER may edit a membership row, so without extra work a person could never correct their own name. Fixing that properly needs care — the naive version would also let someone promote themselves to owner |
| 5b-iii–5h | The rest of the screens | After 5b-ii |
| **—** | ⚠️ **Two things are waiting on YOU** | **The first is your Números questions.** You said on 2026-09-17 they are *not* about how it looks — so they are about what it measures, which reopens área 9 and probably costs a migration. ✅ The undo question was answered the same day: **no undo.** It is in `docs/PLAN.md`'s decisions block and every session re-offers it until you say what they are. ⚠️⚠️ **The second arrived with this session's sizing and is a paperwork fix, not a product question**: the architecture document says the invite function runs under the ordinary permission rules, and the one we actually built cannot — it needs wider powers to do a step the document itself asks for. **Nothing is broken and no code changes either way**; the document is simply wrong about it, and it has said so since 2026-09-13. **The recommendation is to fix that sentence inside `5b.8`**, which already has to reopen the same section for two other reasons |
