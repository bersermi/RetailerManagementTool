...
| **5f-ii** | **The list and the row — the screen, with nothing committed** | ⏸️ **WAITING ON YOU — see the bottom of this table.** This is the list of everything you sell with a search box, and each row carrying its own quantity: the name, the family under it, the price **always with its unit** (`$35.00 / kg`, never a bare `$35.00`), the `−`/`+` and the `...`. **Nothing is saved from this screen** — it is the surface, and the finishing is the next piece. ⚠️⚠️ **WHAT IS WAITING ON YOU IS THE QUANTITY CONTROL, AND IT IS A DISAGREEMENT RATHER THAN A QUESTION.** The design document says a weighed product gets a **number pad and no stepper**; your own words say it gets **both**. **You are almost certainly right and the document is out of date** — but it is the document everything else is built against, so it gets corrected rather than quietly ignored. **One sentence from you and this is clear.** ⚠️ **It changes no database.** |
| **5f-iii** | **The basket sheet and the slide that finishes a sale** | ⏳ **After the two above.** ⚠️⚠️ **THIS IS THE FIRST TIME THIS APP WILL EVER SAVE A SALE** — everything built in the *no signal* work has been tested against made-up data, because nothing in the app has ever had anything to save. ⚠️ **Tapping *Quitar* removes a line straight away** — no *are you sure*, no undo — which you ruled on 17 September; putting it back is two taps on the list behind. **Emptying the whole basket still asks.** ⚠️ **And your two animations, from 21 September**: one when a sale completes and one when the basket is emptied. **The sale one plays the moment you slide, never when the server answers** — so a sale with no signal looks exactly like one with, which is the whole point of how this was built. ⚠️ **It also gets you your first look at the little banner for operations that never went through** — until this job there is no way to make one appear on a real phone. ⚠️ **It changes no database.** |
| **5f-iv** | **The two-tap price change at the counter, and whether it sticks** | ⏸️ **WAITING ON YOU — see the bottom of this table.** Changing a price from the `...` while somebody is standing there, and the setting that decides whether that change **becomes your shop's price** or lasts one sale. ⚠️ **It sticks by default, which is your own decision** — and while the setting is off, the home screen says so, because a price you changed on Monday would otherwise surprise you on Tuesday. ⚠️⚠️ **WHAT IS WAITING ON YOU: can a cashier knock money off ONE sale without touching your price list?** Your ruling of 23 September settled who may change the SHOP's prices — you and your Gerentes — and said nothing about a single line. ⚠️ **This is why the question comes before the button and not after it:** an Empleado tapping a price today is not refused, she is **ignored** — the app says nothing and nothing changes. **Drawing the control before you answer means shipping a button that does nothing.** ⚠️ **It changes no database** — widening who may change prices would be one, and those cannot be quietly undone. |
