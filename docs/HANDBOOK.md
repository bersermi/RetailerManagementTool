...
| **—** | ⚠️ **ONE THING IS WAITING ON YOU, AND IT BLOCKS VENDER** | ⚠️⚠️ **Should the app have an "undo that sale" button?** You answered *"I don't remember what the 15-minute undo stands for"* — that was the question's fault, so here it is plainly. **A cashier rings up the wrong thing and taps undo.** ⚠️ **Nothing is erased**: the app writes a mirror-image entry that cancels the first, both stay in the books, and the stock goes back to what it was. Any given sale can be undone once. ⚠️⚠️ **The "15 minutes" is about WHO, not about a deadline** — a cashier may undo **their own** sale within 15 minutes; **you and your managers can undo anything, any time, with no time limit at all.** *My recommendation: yes, build it. The database already does it and nothing calls it. A mis-rung sale that cannot be undone gets corrected by not recording the next one — and "did everything get recorded" is the exact number your pilot is graded on.* ⚠️ **You already ruled on the other two on 21 September:** no receipt and no customer screen (change calculation deferred, and you asked instead for **a confirmation animation when a sale completes and another when the basket is emptied** — both now part of the transaction screen), and **no expiry date capture**, deriving shelf life from the pilot's own records instead. ⚠️ **That second one has a cost worth knowing: a derived shelf life needs weeks of waste records before it tells you anything**, where a typed date would have worked on day one. |
