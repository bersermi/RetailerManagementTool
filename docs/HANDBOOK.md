...
| **5b-ii-b** | **Inviting someone, and them joining** | **After `5b.7`.** The sheet it adds the button to now exists, and the name work does not change its shape |
| **5b.7** | **Asking people their name when they create their account** | ⚠️ **This is where the next piece of work is**, and nothing is waiting on you for it. ⚠️⚠️ **It is the one job with a deadline that is not a date**: anybody who signs up with an email address before it ships has **no name stored anywhere, and there is nothing to recover it from later**. People who sign in with Google are already fine — Google gives us the name whether we use it or not. It only collects the name; nothing shows it yet. ✅ **You ruled on 2026-09-18 that both a nombre and an apellido are required** — so the form asks in **two boxes**, both of which must be filled, rather than counting the words in one. ⚠️ **It cannot be required of people who sign in with Google**: Google sends one line of text and some accounts genuinely have no surname. Those people are taken as they come, and `5b.8` gives them a way to fix their own name in Ajustes |
