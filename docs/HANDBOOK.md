...
| **5c-ii-b** | **Noticing that the signal came back** | ⚠️ **This is where the next piece of work is** — and as of 21 September it can actually be started, because the measurement it was waiting for was taken that morning. ~~It was the first time the next job could not be started on the day it was named.~~ **It was waiting on a measurement, not on you, and not on any code.** The app needs one way of knowing whether it has a connection, and there are two ways to ask the phone. Which one to use is a reading taken on a real iPhone and a real Android — they have disagreed three ways in one day on this project already — and **your iPhone is not free until the eight-day sign-in reading on 21 September is taken**, because opening the app restarts that clock. ✅✅ **That reading is now in, taken on 21 September on both your iPhone and the sealed emulator, and the answer is YES: after eight days untouched, both apps opened straight to Inicio and nobody had to sign in again.** ⚠️ **It is a checkpoint, not the final answer** — the same two devices get looked at again on 13 October. So your iPhone is released and this job can go ahead. ⚠️⚠️ **One thing changed under it and you should know:** this Mac turns out to have an iPhone SIMULATOR on it now (it did not on 20 September, and that was checked at the time, not assumed). A simulator is not a full substitute — it borrows the Mac's internet and has no mobile data — so it can answer *"does the app notice when the connection drops"* but not *"is this wifi or mobile data"*. **Whether that second question matters is the first thing this job will settle**, and if it does, it is one short session with the phone in hand rather than a reason to wait. It also builds on what `5c.5` measured: a dropped connection costs the app up to 90 seconds before it tries again, so that is the number the timing is designed around rather than a guess |
...
| **—** | ⚠️⚠️ **THREE THINGS ARE WAITING ON YOU, AND THEY BLOCK THE TWO SCREENS THE PILOT NEEDS MOST** | ⚠️⚠️ **These are not new questions. They were raised on 7 September and then sat in a table near the bottom of the plan for fourteen days, outside the one place this project guarantees you get asked again.** Moved on 21 September. **(1) When a sale is finished, does anything print, and does the customer see the screen?** *My recommendation: no receipt and no customer-facing screen — but DO add change calculation, two fields on the last screen of a sale. It is the one thing a cashier does after every cash sale that the app cannot currently help with.* **(2) Does the 15-minute undo get a screen?** *My recommendation: yes. The database already has it and nothing calls it. A mis-rung sale that cannot be undone gets corrected by not recording the next one — and "did everything get recorded" is the exact number your pilot is graded on.* **(3) Does Comprar capture an expiry date when goods come in?** *My recommendation: yes, one optional date field. The database already accepts and stores it — and without it, "what is about to go off" cannot be answered at all, which is the whole input to the waste business.* ⚠️ **All three block work that is coming up soon; the fourth, below, blocks nothing.** |
| **—** | ⚠️ **AND ONE OLDER ONE, WHICH STILL BLOCKS NOTHING** | ⚠️⚠️ **Should we give "that code isn't a shop" its own error number?** Right now the database answers a wrong shop code with the SAME number it uses for *"you're signed out"* — so the app has to guess which happened, and it guesses "wrong code", because that screen can only be reached by somebody who IS signed in. **My recommendation: yes, fix it, as its own small piece of work after the next one.** It is the exact same problem you already ruled on for the other code box on 2026-09-18 — *"cheap now, dearer later"* — and it was worth fixing then. ⚠️ **What saying no costs you:** somebody whose sign-in quietly expires while they are on that screen is told to re-check a code that was fine. Reopening the app puts them right. That is the whole of it, which is why nothing is held up waiting for your answer. ⚠️ **It must not be bundled into the next piece of work** — that one is a different database function, and this project has already recorded why two unrelated fixes in one database change are harder to undo. ⚠️ **The app already has a test that goes RED the day this is fixed**, so it cannot be quietly forgotten. |
