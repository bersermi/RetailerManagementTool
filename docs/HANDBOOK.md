...
| **5f-ii** | **The list and the row — the screen, with nothing committed** | ⚠️ **This is where the next piece of work is** — ✅✅ **you cleared it on 24 September, about four hours after it was raised: *"Amend it to say both."*** **Every product now gets a stepper AND a number pad, one tap apart**, and the design document is corrected to say so. ⚠️⚠️ **AND YOU WENT FURTHER THAN THE QUESTION, WHICH CHANGED THE ANSWER:** I had argued that a product sold by the piece needs no number pad, because there is no half a manojo. **You pointed out that is beside the point — fifteen manojos of cilantro is fifteen taps.** A number pad is not about precision, it is about how many. **That was the right correction and it is now written into the design document, not just the plan.** ⚠️ **One thing I have NOT assumed**: you also said the step *"can be set"*. The step is already yours to set — it is whatever unit you priced the product in, so pricing by the quarter-kilo gives you a 250 g step. **If you meant something separate from the price unit, say so and it is a small addition.** This is the list of everything you sell with a search box, and each row carrying its own quantity: the name, the family under it, the price **always with its unit** (`$35.00 / kg`, never a bare `$35.00`), the `−`/`+` and the `...`. **Nothing is saved from this screen** — it is the surface, and the finishing is the next piece. ⚠️⚠️ **WHAT IS WAITING ON YOU IS THE QUANTITY CONTROL, AND IT IS A DISAGREEMENT RATHER THAN A QUESTION.** The design document says a weighed product gets a **number pad and no stepper**; your own words say it gets **both**. **You are almost certainly right and the document is out of date** — but it is the document everything else is built against, so it gets corrected rather than quietly ignored. **One sentence from you and this is clear.** ⚠️ **It changes no database.** |
