...
| ⚠️⚠️ **ADR-035 §2.11's stack table has no CONNECTIVITY row, and as of today the app has a connectivity dependency — amend it?** | **Nothing. The dependency is already in the tree and the reading that chose it is recorded in this file**; what is missing is the ADR sentence naming it, which is the stale-copy class this repository has recorded eight times. The task marked next may be taken without waiting — it reads the plan, not the stack table | **RECOMMENDED: yes, one row, and fold it into the next task rather than giving it a session.** §2.11's table fixes every other client-wide choice — Expo Router, TanStack Query, Zustand for the cart, the palette, the motion rule — precisely so a junior cannot invent a second one. **`expo-network` is now exactly that kind of choice** and it is the only one with no row. ⚠️ **What it costs to say no:** the next person who needs connectivity finds no ADR sentence, adds `@react-native-community/netinfo` because it is the better-known package, and the app has two connectivity signals that disagree — which is the specific failure the winning row was chosen to prevent, since the loser **never recovered from an outage on iOS**. ⚠️ **What it costs to say yes:** one sentence, and the obligation to strike it honestly if a real iPhone ever contradicts the simulator reading. ⚠️ **The recommendation is deliberately NOT its own task** — [[process-must-not-outgrow-product]]: a one-row ADR amendment that rides along with the module it describes is cheaper than a session, and the module is the thing that makes the row checkable |
...
⚠️ **TWO ARE OWED AS OF 2026-09-22 AND NEITHER BLOCKS ANYTHING** — the SQLSTATE mint,
above, and the ADR §2.11 connectivity row that `5c-ii-b-1` raised the day it chose the
dependency. ~~one remains owed as of 2026-09-22 and it blocks nothing~~ — struck in
lower case deliberately, the rule `5b.8-i`'s row records. ~~two remain owed, and one of them still blocks the pilot's most important
