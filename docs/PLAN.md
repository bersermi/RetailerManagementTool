...
### 📦 Older status-log entries — 2026-09-18 and back, moved out of this file ✅

⚠️ **ARCHIVED 2026-09-20 to [`docs/plan/archive/status-log-through-2026-09-18.md`](plan/archive/status-log-through-2026-09-18.md)** —
4,576 lines, unedited, verified byte-identical on reconstruction. `## Position` had
reached **5,484 lines**: the largest single thing a session read, larger than the
whole of Step 5, and the section the working prompt sends every session to **first**.
Almost all of it was status log — one narrative entry per closed task, forty of them,
growing every session and never shrinking.

**What stayed, and the rule:** the two blocks that carry live obligations
(`⛔ DECISIONS OWED`, `⏳ DATES OWED`) and the most recent working day's entries —
2026-09-19, which is the whole of `5b-iii` and `5b.8-iii`, the lineage the next task
sits in. ⚠️ **The blocks must never be archived**: `plan-handover.sh` requires exactly
one of each **in the live plan**, and a second copy is a second home for one claim.

⚠️ **AND THE CEILING IS NOW CHECKED RATHER THAN REMEMBERED.** `plan-handover.sh`
assertion 11 fails when `## Position` passes **1,400 lines** and names the remedy, so
this section cannot silently grow back — which it would, at one or two entries a
session, and which is exactly how it reached 5,484 without anyone deciding to.

To find an archived entry: `graphify query` first, then
`grep -n '<task-id>' docs/plan/archive/status-log-through-2026-09-18.md`. Every split
guard already reads the live plan **plus** the archive via
`docs/checks/plan-corpus.sh`, so a task row resolves from there exactly as before.
