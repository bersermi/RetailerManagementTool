...
|---|---|---|
| ⚠️ **ADR-035 §2.8's HOME ROW — HALF RULED 2026-09-22, HALF STILL OPEN** | `5d-iv` | ✅✅ **(a) IS RULED: *"Let's drop it for the pilot then."* THE 48-HOUR EXPIRY BLOCK IS WITHDRAWN**, and `5d-iv`'s row records it as withdrawn rather than deferred — the shape ADR-035 §3 used for the two deliverables C1.5 killed. ⚠️ **The ADR edit is held until (b) is ruled**, because both halves are the same row of the same table and two edits to one sentence is how a document grows a contradiction. ~~**(a) the 48-hour expiry block has nothing to put in it.**~~ §2.8 (amended 2026-09-17) puts *"anything expiring within 48h"* on Inicio, above anything tappable. On **2026-09-22 you ruled *"we will not capture expiry date for now"***, so `0018`'s tier 1 — the operator's own date — never fires. ⚠️ **Tier 2 still could**: `record_purchase` seeds a batch's expiry from `product_family.default_lifespan_days` with nothing typed at the counter — **but only when `track_expiry` is true on the family**, which is `false` by default (`0002:78`), and **C8.9's four-field create asks for neither**. So every family the pilot creates has both off and the block is empty on every day of the pilot. ⚠️ **An always-empty panel above everything tappable is the worst of the three outcomes** — it is the app talking about its own plumbing — and `7e` was rewritten on 2026-09-21 to DERIVE shelf life precisely because you chose not to capture it. ✅ **RULED AS RECOMMENDED: §2.8 drops the 48-hour block for the pilot**, and `7e` answers the question later out of records the shop already produces. ⚠️ **If you would rather have it now**, the cost is one human step away from the counter: a lifespan field on `Editar` in `5e`, typed once per family, and tier 2 then fires on its own. **(b) AND §2.8 ALSO SAYS PERMANENT FAILURES *"DO NOT APPEAR ON HOME AT ALL"*** — while `5c-iv-b`'s dead-letter banner, merged 2026-09-22, overlays **every** screen including Inicio, on your own C11.9 constraint. ⚠️⚠️ **That is a live disagreement, and `CLAUDE.md` says the ADR wins** — so a session building Inicio and reading §2.8 literally would take yesterday's banner off Home and be obeying the rules while doing it. ✅ **RECOMMENDATION: amend the same row to say the banner is the one permanent failure Home does carry**, fenced to manager-and-above, which is what C11.9 asked for and what is already built. ⚠️ **Neither half is a migration and neither blocks the next task**: `5d-i`, `5d-ii` and `5d-iii` never touch Inicio. It costs you one ruling and me one ADR edit |

⏳⏳ **ONE IS OWED AS OF 2026-09-22, IT ARRIVED WITH `5d`'s SIZING, AND IT BLOCKS ONLY THE
LAST OF THAT SPLIT'S FOUR CHILDREN.** ~~nothing is owed as of 2026-09-22, and the empty table
above is deliberate — for the third time in this project's life.~~ — ⚠️ **struck in lower case
deliberately, the rule `5b.8-i`'s row records.** ⚠️ **TWELVE decisions have now been parked in
this block, eleven of them are ruled, and this is the twelfth.** ~~eleven decisions have now been
parked and cleared in this block.~~
...
✅ **ÁREA §2.8 (a) — RULED 2026-09-22: *"Let's drop it for the pilot then."* THE
48-HOUR EXPIRY BLOCK IS WITHDRAWN FROM INICIO**, and `5d-iv`'s row says *withdrawn*
rather than *deferred* — the shape ADR-035 §3 used when C1.5 killed the shared till.
⚠️ **The ADR edit itself is HELD until the (b) half is ruled**: both are the same row
of the same table, and editing one sentence twice is how a document grows a
contradiction. ⚠️ **The cost, stated when it was parked and unchanged by the ruling:**
`7e`'s derived shelf life needs weeks of the shop's own records before it says
anything, where a typed date would have worked on day one — and the owner has now
chosen that trade twice, on 2026-09-21 and again here.
