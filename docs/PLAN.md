...
|---|---|---|
| ⚠️⚠️ **THE QUANTITY CONTROL IS ONE CONTROL OR TWO, AND THIS IS AN ADR AMENDMENT RATHER THAN A QUESTION — THE ADR AND YOUR OWN WORDS DISAGREE.** ADR-035 §2.8 says *"stepper for discrete units, decimal keypad for weight and volume… **Never the same control for both**"*, and §2.11 names the primitive **`QtyInput` — the stepper/keypad SWITCH**. Your specification of 2026-09-23 puts **both on one weighed line**: *"$45/250gr, when selling he can use the stepper to go 250 → 500 → 750 → 1000. But he can also tap to enter 288gr if needed."* **C3.8 has said the same since the área 3 interview**, so the plan has disagreed with the ADR here for weeks and nothing looked. ⚠️ **The ADR wins by this repository's own rule, so the screen cannot simply be built from the plan** — and the amendment is the cheap half: **two sentences, no code, no migration, no deployed consequence.** ✅ **RECOMMENDATION: amend §2.8 and §2.11 to say BOTH on a weighed line, because you have specified it twice and the shop is the reason** — a `250g` step is how a pollería sells, and `288g` is what the scale actually says. **Nothing is lost by it:** the SWITCH survives in the form that was always the point — *a discrete unit gets no keypad*, because there is no 288th of a `pza` | `5f-ii` | The sizing under Step 5, and that child's own row |
| ⚠️⚠️ **DOES THE `...` OFFER A CASHIER A PRICE OVERRIDE FOR THIS SALE ONLY — one that never writes `price_list`?** The word the C3.17 ruling deliberately did not give, now sitting on the child that draws the control. You ruled *"leave the fence as is"* on 2026-09-23, so the shop's price list stays manager-and-above — and a cashier's PATCH of it is **not even refused loudly**: the row is invisible to her, so PostgREST answers 200 with an empty array (measured, `5e-iii-a`). **That settles who may change the SHOP's price and says nothing about ten pesos off one line.** ⚠️ **It is entangled with C3.16's *a price change persists by default***: a sale-only override is the one price change that must NOT persist, so it is either a second affordance or a per-tap choice on the same one. ✅ **RECOMMENDATION: yes, and make it the ONLY price control a cashier is shown** — she is the person standing there when a regular asks, the ledger records the real figure either way because `sale_line` snapshots the price it was sold at, and **nothing reaches `price_list`, so the fence you kept is untouched.** The alternative is a cashier who has to fetch a manager to discount a kilo of chicken | `5f-iv` | That child's own row, and C3.17's closing paragraph |

⚠️⚠️ **TWO ARE OWED AS OF 2026-09-24, BOTH CAME OUT OF `5f`'s SIZING, AND NEITHER BLOCKS THE
TASK MARKED NEXT.** ⚠️ **The first is a CONTRADICTION rather than a question — the second of
those in this block's life, after C3.17 — and it is the first one here that asks for an ADR
AMENDMENT on a sentence the ADR has carried since 2026-08-14.** ⚠️ **NINETEEN decisions have
now been parked in this block.**
...
⚠️⚠️ **EIGHTEEN ENTRIES OF 2026-09-22 ARE ARCHIVED, IN FOURTEEN CUTS, TO
...
and it is the first time this block has been cut twice in one sitting. ⚠️ **There is no
2026-09-22 entry left in `## Position`**: that day is now wholly in its own file.
⚠️⚠️ **The archive is no longer strictly oldest-first and its own headings say so**:
three cuts came off this block's head and eleven off its foot.
