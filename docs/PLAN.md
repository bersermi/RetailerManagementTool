...
|---|---|---|
| ⚠️⚠️ **DOES A CASHIER CHANGE A PRICE? C3.17 SAYS YES AND THE DATABASE HAS SAID NO SINCE 2026-08-26.** Widening it is a migration; narrowing C3.17 is a sentence | `5e-iii-b` — the amber dash, and who may be shown `Editar` at all. ⚠️ **Not** the task marked next | **The contradiction, in two lines.** C3.17 is your own ruling: *"We'll perfectionate on the role capabilities, for now let's allow anyone to change the price"* — and it closes *"it is client-side only; no schema depends on it."* **`0002` makes `price_list_insert`, `price_list_update` and `price_list_delete` all `has_role(workspace_id, 'manager')`**, so a cashier is refused by Postgres with a bare `42501` and no sentence. ⚠️ **What it costs each way.** With C3.16 — *a price change persists by default* — the two together mean a cashier's counter discount **permanently rewrites the shop's price list**; you were shown that consequence for the CLIENT and took it, not knowing the database already refused it. ⚠️⚠️ **My recommendation: leave the fence where `0002` already put it, and narrow C3.17 to *a cashier may change a price for THIS SALE* — re-asked at `5f`, which is the screen where a counter price actually happens.** The usual *cheap now, dearer later* points the other way here: doing nothing costs nothing and is reversible, and widening the policy is an append-only migration that merges automatically. ⚠️ **What it blocks in `5e-iii-b`**: amber on La Familia means *the fix is one tap away*, and whether that is true depends entirely on who is holding the phone — and, behind that, whether a cashier is shown the form at all, which is the `5e-ii` question asked again about a screen she has more reason to open. ⚠️⚠️ **AND IT IS NOT NAMED AS BLOCKING VENDER, DELIBERATELY:** the counter price change and its persistence setting are ONE deliverable of a task that has not been sized, and a Blocks cell naming the whole of it would stop the sizing that is supposed to route this. **The sizing session must park that deliverable against this row rather than build it** |

⚠️⚠️ **ONE IS OWED AS OF 2026-09-23, IT CAME OUT OF `5e-iii`'s SIZING, AND IT IS THE FIRST
ONE HERE THAT IS A CONTRADICTION RATHER THAN A QUESTION — the owner's own C3.17 against the
schema that has been applied since 2026-08-26. ⚠️ **It does NOT block the task marked next.**
⚠️ **FIFTEEN decisions have now been parked in this block.**
...
`5f` IS THE NEXT TASK: VENDER, AND ITS OWN ROW SAYS TO SIZE IT FIRST.**
