...
| ⚠️⚠️ **When a sale is finished, does anything PRINT, and does the customer ever see the screen?** (round-three área 5, never asked) | **`5h` — Vender's commit step**, and with it the tap budget §5 makes a stop-the-pilot rule | **RECOMMENDED: no receipt, no customer-facing screen, and change calculated on the device.** A printed receipt needs hardware nobody has costed, and §6 defers CFDI deliberately as *"a different product; would consume this one"*. ⚠️ **The half worth building is CHANGE**: two fields on the commit sheet — amount tendered, change owed — computed on the device from a total §2.6 already computes there. That is the one thing a cashier does after every cash sale and the one thing the app currently cannot help with. ⚠️ **Why it blocks rather than waits:** the commit sheet is the last screen of `5f`'s shared surface, so a decision taken after `5h` ships is a rewrite of the highest-traffic surface in the product |
| ⚠️⚠️ **Does the 15-minute void get a screen, and who reaches it?** (round-three área 6, never asked) | **`5h` — Vender**, and the completeness number the pilot is graded on | **RECOMMENDED: yes — one undo on the sale just committed, fenced exactly where `0021` already fences it** (the person who rang it, within 15 minutes). ⚠️ **`0021` is applied and has NO caller**, so this is a screen over a finished function rather than new risk. ⚠️⚠️ **THE PILOT ARGUMENT IS THE STRONG ONE: a mis-rung sale with no undo is corrected by NOT RECORDING THE NEXT ONE**, or by a staff member who stops trusting the app — and §5 grades the pilot on completeness within 5%, so the commonest error in a shop lands directly on the one number that decides whether the pilot passed |
| ⚠️⚠️ **Does Comprar capture an EXPIRY DATE per line?** (round-three área 7, never asked) | **`5g` — Comprar**, and the *"what expires soon"* read the waste hook rests on | **RECOMMENDED: yes, optional per line, one field.** ⚠️ **The server half is already built and free**: `record_purchase` accepts `expiry_date` per line today and `stock_batch.expiry_date` stores it, with FEFO already ordering on it. ⚠️⚠️ **WITHOUT IT THE TIER-2 BUSINESS HAS NO INPUT** — buying stock before it becomes waste requires knowing what is close to expiring, and nothing else in the schema can answer that. It is one date field on a receiving line, and it is the cheapest thing in this document relative to what it unlocks |

⚠️⚠️ **FOUR ARE OWED AS OF 2026-09-21, AND THREE OF THEM BLOCK THE PILOT'S TWO MOST
IMPORTANT SCREENS.** ~~one is owed as of 2026-09-19, and it blocks nothing~~ — struck
...
| **7e** | ⚠️⚠️ **WHAT EXPIRES SOON — THE ONE READ THE TIER-2 BUSINESS CANNOT RUN WITHOUT, AND IT IS NOT ONE OF ÁREA 9's THREE QUESTIONS.** Stock inside its last days, by variant and location, with quantity and its cost basis. ⚠️ **The expensive half is already built**: `stock_batch.expiry_date` exists, `record_purchase` accepts it per line, and FEFO already orders on it. **What is missing is the capture field (⛔ DECISIONS OWED, área 7) and this query.** ⚠️ **Added 2026-09-21 as a requirement of the business model rather than of §2.9** — it is the read that turns "analyse waste patterns" into "collect it before it is waste" | `M` | The expiry decision, then `5g` |
