...
|---|---|---|
| ⚠️⚠️ **ÁREA 6 — *MISTAKES*. What happens when a shopkeeper rings up the wrong thing, and who may undo it?** | **`5h-ii`** — the undo, and nothing else. ⚠️ **`5h-i` is the ROUND that answers this and is NOT blocked by it.** ⚠️ **The task marked next is unaffected**: it is the magnitude warning, which reads purchase history and touches no reversal. ⚠️⚠️ **AND THIS CELL DELIBERATELY DOES NOT NAME IT** — `4.6b` wrote a next-task id into a *Blocks* cell on 2026-09-14 in a sentence whose whole purpose was to say it was NOT blocked, and assertion 7c read the name and refused the task. **A Blocks cell is machine-read, so it is not a place to write reassurance.** | ⚠️⚠️ **THIS ROW IS A RE-PARK AND THAT IS THE FINDING: IT WAS MOVED HERE ON 2026-09-21 AND IS NOT HERE.** The third-round table still says *"MOVED TO ⛔ DECISIONS OWED 2026-09-21 — read it there, not here"*, and there was nothing there to read. The block emptied for the thirteenth time on 2026-09-25 and this question went with it, which is **the first time the one block whose entire purpose is to re-offer open decisions every session has lost one.** ⚠️ **Nothing could see it**: the guard checks the block is present, singular, and that every row names what it blocks — it cannot check that a question which belongs in it is there. ⚠️ **Found by `5g-iii` trying to mark `5h` as the next task and reading its gate cell.**
