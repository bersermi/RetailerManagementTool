...
|---|---|---|
| ⚠️ **SHOULD ADR-035 §3 SAY THAT THE `src/ui/` CONVENTIONS ARE WRITTEN AT `5h.5`?** A one-line amendment, or a ruling that it should not be made. | **Nothing takeable — it blocks no task.** What it blocks is the ADR staying accurate: §3 still names both halves at the interstitial that closed on 2026-09-18, and only the `src/api/` half was written there | ⚠️⚠️ **THIS IS A RECORD-KEEPING DECISION WITH A DEADLINE OF NOBODY, AND IT IS EXACTLY THE KIND THIS REPOSITORY LOSES.** The reasoning is in the closing entry of 2026-09-18 under `## Position`, in full: `app/src/ui/` did not exist, the owner's ruling of 2026-09-13 refuses conventions written against primitives nobody has drawn, and ADR-035 §2.10 says the claim is *"about ORDER RELATIVE TO STEP 6, not about the letter"* — so `5h.5` is an upholding. ⚠️ **What is NOT settled is whether the ADR should say so in its own words.** Amendments to it have always been made on the owner's instruction; the model will not amend it alone, and CLAUDE.md's *"the ADR wins"* means a future session reading §3 literally would conclude the plan is the bug. ⚠️ **Three files already cross-check this deferral and the gate reads two of them**, so the ADR is the copy with no instrument — the shape of six of this repository's recorded stale-copy defects. **Either answer closes it: amend the line, or rule that §3's letter stands and the plan is wrong.** |
