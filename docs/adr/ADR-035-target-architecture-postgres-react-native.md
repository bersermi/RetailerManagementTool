...
- **Revised:** 2026-09-25 — **§2.11 gains an EXPORT row**, folded into plan task `5g-iii`
  rather than given a task of its own: the arrangement `5c-ii-b-2` used for the connectivity
  row, and for its reason — *a stack-table row describing a module the same sitting builds is
  a sentence somebody can check*. ⚠️⚠️ **IT ARRIVES FROM A REQUIREMENT AND NOT FROM A REVIEW.**
  The decision maker specified `Costos` on 2026-09-24 and ended the sentence with *"You can
  share the 'view' as a PDF"* — and **nothing in this app had ever produced a file**, so there
  was no row to read and no precedent to follow. §2.11's table fixes every other app-wide
  choice precisely so a junior cannot invent a second one; this was the last capability with
  none.
  ⚠️⚠️ **THE ROW CARRIES ITS BOUND AND THE BOUND IS THE HARSHEST IN THAT TABLE: NO WORKFLOW IN
  THIS REPOSITORY COMPILES THIS APP.** `app/ios/` is gitignored and generated, `app.yml` is a
  typecheck plus a Vitest suite plus the document guards, so **a native dependency can be added
  and every check stays green** — which is not a hypothetical: `5c-ii-b-2`'s library swap left a
  Pods target pointing at a deleted file and the next device build failed on source nobody had
  edited. A row stating a choice without its evidence is the stale-copy shape this repository
  has recorded ten times.
  ⚠️ **What this revision buys back, and it is most of the feature:** the DOCUMENT was split
  from the SHARING. `@/export/costsPdf` is pure and returns an HTML string, so
  `app/test/costs-pdf.test.ts` asserts every figure, every date, every provider name, the HTML
  escaping of a name a shopkeeper actually types, and — the one that matters — that the cheapest
  delivery is drawn **lowest**, because a flipped axis is the only geometry bug that still looks
  like a working chart. **Only two native calls are outside**, in `@/export/share`.
  ⚠️ **No schema is applied by this revision.** What shipped with it is client code: one data
  module, one pure document builder, one native binder, a screen, 80 new assertions, and a live
  contract check. ⚠️ **What is NOT discharged: a device build.** The row's evidence is an `R9`
  reading on the decision maker's own phone, and until one is taken this row states a choice
  whose native half has been compiled by nobody.
- **Revised:** 2026-09-25 — **§2.11's EXPORT row, hours after it was added, on the decision maker's
  instruction.** *"The Chart doesn't survive the Web view. Let's turn the PDF export into a nice
  tabular view. A chart in PDF doesn't make a lot of sense."* **The document is a TABLE.** The row's
  first version said the file carried the same chart as the screen; it does not, and the sentence is
  corrected here rather than left to be discovered — the stale-copy defect this repository has recorded
  ten times, caught inside a day this once.
  ⚠️⚠️ **IT IS THE CLEANEST VINDICATION OF `R9` THIS DOCUMENT HAS: TWENTY-TWO ASSERTIONS PROVED THE PDF
  CORRECT AND THE PDF WAS STILL WRONG.** Every figure, every date, every provider name, the HTML
  escaping of a name a shopkeeper really types, and the one that mattered — that the cheapest delivery
  was drawn lowest. **Nothing in it was false. It was not what a PDF is for.** §2.11 fences rendering
  out of this repository precisely because a suite cannot hold that distinction, and this row's bound
  was written as *an `R9` reading on a real device* before anybody had looked. **A check that had
  claimed otherwise would have been the expensive kind of green.**
  ⚠️ **The generalisable half, worth keeping for the next export anybody adds:** a chart earns its place
  through interactions — hover, tap, zoom, re-scale — and **a printed or forwarded page has none of
  them**, so the rows underneath are the thing a person actually checks a notebook against.
  ⚠️ **`plotted` does not move.** The geometry stays in `@/api/costs` and keeps serving the screen, which
  the decision maker confirmed in the same message reads fine. **No schema is applied by this revision
  and no dependency changes**; the plan's `5g-iii-a` owns the code.
...
| **Export** | **`expo-print` + `expo-sharing`, read through ONE module** — added 2026-09-25 | It is the first thing this app produces that LEAVES the phone, and the only stack row whose correctness no workflow here can check. ⚠️⚠️ **THE BOUND FIRST, BECAUSE IT IS WHAT MAKES THIS ROW HONEST: nothing in CI compiles this app.** `app.yml` is a typecheck, a Vitest suite and the document guards; `app/ios/` is generated by `expo prebuild` and is not committed. So **a native dependency can be added and every check stays green** — which `5c-ii-b-2` proved when the netinfo → `expo-network` swap left a Pods target pointing at a deleted path and the next device build failed on a file nobody had touched. **`pod install` before `xcodebuild`, and the row's evidence is a device build rather than a green tick.** ⚠️⚠️ **AND THE DOCUMENT IS A TABLE, NOT A CHART — AMENDED 2026-09-25 ON THE DECISION MAKER'S INSTRUCTION, HOURS AFTER THIS ROW WAS WRITTEN.** His words, from his own phone: *"The Chart doesn't survive the Web view. Let's turn the PDF export into a nice tabular view. A chart in PDF doesn't make a lot of sense."* ⚠️ **THIS ROW'S FIRST VERSION SAID THE FILE CARRIED THE SAME CHART AS THE SCREEN, AND IT NO LONGER DOES.** The geometry (`plotted` in `@/api/costs`) stays where it is and keeps serving the screen, which he confirmed reads fine; what leaves the document is the renderer over it. ⚠️⚠️ **THE REASON IS THE INTERESTING PART AND IT GENERALISES: a chart earns its place through interactions a printed or forwarded page does not have**, and the rows underneath are what a person checks a notebook against. **`5g-iii-a` SHIPPED IT the same day** — the renderer, the legend and the axis column are gone, the document is provider rows against day columns, and `app/test/costs-pdf.test.ts` now asserts that **not one `SERIE` hex appears in the file at all**, which is the claim that would catch a chart coming back. ⚠️⚠️ **AND IT MEASURED THE PAGE, WHICH NOBODY HAD: `printToFileAsync` defaults to `width: 612, height: 792` — US Letter at 72 PPI — and this app passes neither.** `5g-iii` had sized its plot at 660 px against a comment claiming A4 at 96 dpi, so the chart was about 130 px wider than the page it was printed on; *"doesn't survive the Web view"* may well have been arithmetic rather than taste. **The drawable width is 564 px**, which is why the table cuts its days into blocks of six rather than running off the page — a matrix is one column per delivery day, and a product bought weekly for three months has twelve of them. ⚠️⚠️ **AND THIS IS THE CLEANEST VINDICATION OF `R9` IN THIS DOCUMENT: TWENTY-TWO ASSERTIONS PROVED THE DOCUMENT CORRECT AND THE DOCUMENT WAS STILL WRONG.** Nothing in it was false — it was not what a PDF is for. **A check that had claimed otherwise would have been the expensive kind of green**, which is why this row's bound was written as an `R9` reading before anyone had looked. ⚠️ **What a suite here CAN prove, and does: the DOCUMENT.** `@/export/costsPdf` is pure and returns an HTML string — `app/test/costs-pdf.test.ts` reads every figure, every provider name, the escaping of a shopkeeper-typed `Ferretería "El Águila" & Hijos`, and (while the chart was in it) that the cheapest delivery was drawn LOWEST — a flipped axis being the one geometry bug that still looks like a working chart. **What is left outside is two native calls** in `@/export/share`, which is §2.11's connectivity arrangement applied again: one module holds the import, everything else asks it. ⚠️ **Why these two and not one:** `expo-print`'s `printToFileAsync` is the only thing in the Expo SDK that makes a PDF at all, and React Native's built-in `Share` takes a `url` on iOS and a `message` only on Android — so a shopkeeper on either of C1.1's two low-end Androids would get a link she cannot open. Both are first-party Expo modules on the same `~57.0.x` line as every other dependency here; the third-party alternatives are unmaintained on this RN version. ⚠️ **No file is persisted and `expo-file-system` is deliberately absent**: `printToFileAsync` writes to the CACHE, the document is derived from the ledger on every tap, and a stale copy is worse than no copy. ⚠️ **A cancelled share sheet is indistinguishable from a sent one on both platforms**, so the module reports `shared` for both rather than claiming a confirmation the OS never gave — §2.6's rule about never showing a confirmation the source did not give, pointed at a share sheet |
